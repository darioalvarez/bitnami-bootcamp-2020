...
if [[ "${CKAN_POSTGRESQL_SERVICE_HOST}" = "" ]]; then
    psql_host=${POSTGRESQL_HOST}
else
    psql_host=${CKAN_POSTGRESQL_SERVICE_HOST}
fi


if [[ "${CKAN_REDIS_MASTER_SERVICE_HOST}" = "" ]]; then
    rds_host=${REDIS_HOST}
else
    rds_host=${CKAN_REDIS_MASTER_SERVICE_HOST}
fi


PGPASSWORD=${POSTGRESQL_PASSWORD} psql -h ${psql_host} -U postgres -c \
...
sed -i "s!sqlalchemy.url = postgresql://ckan_default:pass@localhost/ckan_default!sqlalchemy.url = postgresql://${POSTGRESQL_DATABASE_USER}:${POSTGRESQL_DATABASE_USER_PASSWORD}@${psql_host}/${POSTGRESQL_CKAN_DATABASE}!1" /etc/ckan/default/development.ini
...
sed -i "s!#ckan.redis.url = redis://localhost:6379/0!ckan.redis.url = redis://${rds_host}:${REDIS_PORT}/0!1" /etc/ckan/default/development.ini
...
sed -i "s!#ckan.datastore.write_url = postgresql://ckan_default:pass@localhost/datastore_default!ckan.datastore.write_url = postgresql://${POSTGRESQL_DATASTORE_USER_WRITE}:${POSTGRESQL_DATASTORE_USER_WRITE_PASSWORD}@${psql_host}/${POSTGRESQL_CKAN_DATASTORE}!1" /etc/ckan/default/development.ini
sed -i "s!#ckan.datastore.read_url = postgresql://datastore_default:pass@localhost/datastore_default!ckan.datastore.read_url = postgresql://${POSTGRESQL_DATASTORE_USER_READ}:${POSTGRESQL_DATASTORE_USER_READ_PASSWORD}@${psql_host}/${POSTGRESQL_CKAN_DATASTORE}!1" /etc/ckan/default/development.ini
